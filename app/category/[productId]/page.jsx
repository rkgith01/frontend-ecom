...
const Category = () => {
  ...
  const handleChange = (e)=> {
    ...
    setselectedSubCat(ifChecked ? [...selectedSubCat, value]: selectedSubCat.filter(item => item !== value))
  }
        ...
        <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
         <BreadCrumbs path={`/category`} />
          ...
          <div className='float-right justify-center w-[710px] mt-5 h-[500px]'>
            ...
            <img src="/catlogo.jpg" alt='categories photo' className='w-[100%] h-[350px] obeject-contain'/>
          </div>
  
          {/* Header of categories */}
          <header>
            <h1 className="mt-7 text-xl font-bold text-gray-400 sm:text-3xl">Product Categories</h1>
         ...
         {/* bigger container to style the products and sorting col  */}
          <div className='w-full flex gap-8'>
            ...
            {/* Filter sections */}
            {!isFiltersOpen && (
              <div className="space-y-4 lg:block mt-2"  style={{
                position: 'sticky',
                ...
                <div className="overflow-hidden rounded border flex flex-col items-start p-4 border-gray-300 [&_summary::-webkit-details-marker]:hidden">
                  ...
                  {data?.map((item) => (
                    <label htmlFor={item.id} className="flex items-center gap-2"  key={item.id}>
                     <input type="checkbox" id={item.id} className="mt-1 h-6 w-6 text-indigo-600" value={item.id} onChange={handleChange}/>
                     <span className="text-sm font-medium text-gray-400"> {item?.attributes.title} </span>
                  </label> 
                  ))}
                ...
                <span className="flex cursor-pointer items-center justify-between gap-1 p-4 text-gray-200 transition text-sm font-medium"> Price </span>
                  <header className="flex items-center justify-between p-4">
                    <span className="text-sm text-gray-700"> The highest price is $1000 </span>
                  ...
                  <div className="border-t border-gray-200 p-4">
                    <label htmlFor="FilterPriceRange" className="block text-sm text-gray-400">
                      Price Range
                    </label>
