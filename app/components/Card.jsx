    ...
    <div>
        ...
        <div className="relative bg-white pt-3">
          ...
          <div className="mt-1.5 flex items-center gap-1.5 text-gray-900">
            ...
            <p className="text-gray-500 line-through">{`$${item?.attributes?.price + 20}`}</p>
            <p className="">{`$${item?.attributes?.price}`}</p>

            {item?.attributes?.isNew && <p className="absolute right-2 text-right text-xs uppercase tracking-wide">New Arrival</p>}
          </div>
        </div>
