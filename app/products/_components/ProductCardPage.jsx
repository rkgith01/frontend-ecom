...
import { Heart, MinusCircle, PlusCircle, RefreshCcw, Shapes, ShoppingBag } from 'lucide-react';
  ...
  return (
    <div className="p-2 py-32 px-10 md:px-38   rounded-lg">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 justify-center">
        ...
        {mainImageUrl ? 
        <div >
          <img src={imageOptions[selectedImage] ? imageOptions[selectedImage] : mainImageUrl} alt="Banner" width={820} height={350} className="rounded object-coversm:h-[300px] md:h-[600px] lg:h-full" />
        </div>
        :  
        <div className='rounded w-[550px] h-[650px] sm:w[450px] bg-slate-300 animate-pulse sm:float-left object-cover'>
          {loading && <p className='items-center justify-center text-center  text-gray-900'>
            <RefreshCcw size={50} className=" animate-spin text-gray-900 sm:ml-[10rem] sm:mt-[10rem] lg:ml-[15rem] lg:mt-[15rem]"/>
            </p>}
        </div>
        }
        ...
        {imageOptions.map((option, index) => (
          <div key={index}>
            <img src={option} alt={`Option ${index + 1}`} width={80} height={80} className="rounded-md object-cover" onClick={() => setSelectedImage(index)} />
          </div>
        ))}
      </div>
        {/* Project Info */}
        <div className=" ml-2 flex flex-col justify-center cursor-pointer mb-5">
          ...
          <div className='flex flex-col'>
            ...
            <h2 className="border-b-2 w-[50%] text-xl lg:text-2xl font-medium mb-3">{itemTitle}</h2>
            
            <h2 className='text-lg lg:text-xl font-medium line-through'>${oldPrice ? oldPrice : "Loading"}</h2>
            <h2 className="text-lg lg:text-3xl font-medium mt-4 lg:mt-0">            
              ${pricing}
            </h2>


            <p className="capitalize border w-[100px] p-2 text-sm lg:text-[1rem] italic line-clamp-2 lg:line-clamp-3 mt-2">{category}</p>
            <p className="leading-loose text-sm lg:text-[1rem] italic line-clamp-2 lg:line-clamp-3 mt-2">{itemDescription}</p>
            ...
            {/* Add to Cart Button */}
            <div className="mt-5 flex  justify-center items-center gap-2 bg-slate-800 p-3 w-[50%] rounded
            hover:bg-slate-600 cursor-pointer" onClick={() => 
              dispatch(
                addToCart({
                  ...products,
                  id: data.id,
          ...
          <div className='flex flex-col gap-3 cursor-pointer'>
                  ...
                  <div className="mt-5 flex items-center gap-3 cursor-pointer">
                  <h1 className='text-uppercase underline'>
                    FAQ
                  </h1>
                  </div>
          </div>
